...
class PlayerUpdateView extends StatefulWidget {
  ...
  @override
  _PlayerUpdateViewState createState() => _PlayerUpdateViewState();
}
    ...
    if(mounted) {
      setState(() {
        ...
        if (widget.playerIndex < 2) {
          ...
          _players = playersApi.where((player) =>
              player.position.contains("MIDFIELDER")).toList();
        } else {
  ...
  DataRow getRow(int index) {
    ...
    return DataRow.byIndex(
      onSelectChanged: (bool) {
        ...
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (BuildContext context) {return TeamDisplayView(players: _players, selectedPlayers: _selectedPlayers,);}));
      },
        index: index,
        cells: <DataCell>[
